...
const ProgrammingLessonCard = ({ item }: Props) => {
	...
	const content = (
		<>
			<div className="flex items-start justify-between gap-3">
				<h3 className="text-xl font-semibold tracking-tight text-[#F1F1F1]">
					{item.name}
				</h3>
				{difficultyLabel && (
					<span
			...
			<p className="line-clamp-3 min-h-[4.75rem] text-sm leading-6 text-[#A9A39A]">
				{item.description}
			</p>
			...
			{item.chips.length > 0 && (
				<div className="flex flex-wrap gap-2">
					{item.chips.map((chip) => (
						<span
							key={chip.id}
							className="rounded-full border border-[#49453E] bg-[#26241F] px-3 py-1 text-xs font-medium text-[#B8B09F]"
						>
							{chip.label}
						</span>
					))}
	...
	const cardClassName =
		"flex h-full min-h-[17rem] flex-col gap-4 rounded-2xl border border-[#3B3B3B] bg-[#222222] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] transition-colors hover:border-[#4A4A4A]";

	if (!item.isAvailable) {
		return (
			<article className={cn(cardClassName, "opacity-75")}>
				{content}
				<div className="pt-2 text-xs font-medium uppercase tracking-[0.16em] text-[#C58B72]">
					In curand
				</div>
			</article>
		);
