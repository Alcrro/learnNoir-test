			...
			{!isModal ? (
				<div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-6xl flex-col gap-6">
					...
					<div className="grid flex-1 gap-6 lg:grid-cols-[1.05fr_0.95fr]">
						<section
							className="order-2 overflow-hidden rounded-4xl border border-(--border) bg-(
						--bg-card) p-6 shadow-sm sm:p-8 lg:order-1 lg:p-10"
						>
								...
								<div className="space-y-5">
									...
									<div className="space-y-4">
										...
										<p className="max-w-2xl text-base leading-7 text-(--text-secondary)">
											The UI uses your current color system, adapts to dark and light mode,
											keeps mobile spacing clean and makes redirects and errors feel
											predictable instead of abrupt.
										</p>
									</div>
								</div>
								...
								<div className="grid gap-4 sm:grid-cols-3">
									<div className="rounded-3xl border border-(--border) bg-(--bg-secondary) p-4">
										...
										<p className="mt-2 text-sm leading-6 text-(--text-secondary)">
											Errors, success and redirect countdowns are explicit and easy to
											understand.
										</p>
									</div>

									<div className="rounded-3xl border border-(--border) bg-(--bg-secondary) p-4">
										<div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-(--teal-bg) text-(--teal-text)">
										...
										<p className="mt-2 text-sm leading-6 text-(--text-secondary)">
											Surfaces, borders and contrast stay balanced across both theme modes.
										</p>
									</div>

									<div className="rounded-3xl border border-(--border) bg-(--bg-secondary) p-4">
										<div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-(--amber-bg) text-(--amber-text)">
										...
										<p className="mt-2 text-sm leading-6 text-(--text-secondary)">
											The form stays first on small screens, without cramped spacing or
											broken hierarchy.
										</p>
									</div>
								</div>
							...
							{isLoading ? (
								...
								<div className="flex h-full flex-col justify-center rounded-[28px] border border-(--teal-border) bg-(--teal-bg) p-6 text-(--teal-text)">
									...
									<p className="mt-3 text-sm leading-6 opacity-90 sm:text-base">
										We detected an active session, so there is no reason to keep you on
										the auth screen. Redirecting to your dashboard in {redirectCountdown}{" "}
